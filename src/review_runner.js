  ...
  <div class="container">
    ...
    <div class="tabs-bar">
      <div class="tabs">
        ...
        <button class="tab-btn" data-tab="readme-loops">
    ...
    <!-- Readme Loops Tab -->
    <div id="readme-loops-content" class="tab-content">
      <div class="panel">
        <div class="panel-header">
          <div class="panel-title">
            <svg style="width: 18px; height: 18px; stroke: var(--accent-color); fill: none;" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
            Active Repository README Refinement Loops
          </div>
          <span class="badge badge-neutral" id="readme-loop-count">0 items</span>
        </div>
        <table id="readme-loop-table">
          <thead>
            <tr>
              <th style="width: 35%">Repository</th>
              <th style="width: 25%">Branch / Head</th>
              <th style="width: 15%">Active PR</th>
              <th style="width: 25%">Iterative Feedback Turns</th>
            </tr>
          </thead>
          <tbody id="readme-loop-tbody">
            <!-- Dynamic rows -->
          </tbody>
        </table>
        <div id="readme-loop-empty" class="empty-state" style="display: none;">
          <span class="empty-icon">📖</span>
          <div class="empty-title">No active README loops</div>
          <div class="empty-desc">No repositories currently have an active README audit loop or refinement branch.</div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    function renderAll() {
      if (activeTab === 'community-prs') {
        ...
        renderPersonalPRs();
      } else if (activeTab === 'readme-loops') {
        renderReadmeLoops();
    ...
    // Render Readme Loops Tab
    function renderReadmeLoops() {
      const tbody = document.getElementById('readme-loop-tbody');
      tbody.innerHTML = '';

      const repos = data.readmeState && data.readmeState.repositories ? Object.keys(data.readmeState.repositories) : [];

      const filtered = repos.filter(repoWithOwner => {
        const matchRepo = (repoFilter === 'all' || repoWithOwner === repoFilter);
        const matchSearch = !searchQuery || repoWithOwner.toLowerCase().includes(searchQuery);
        return matchRepo && matchSearch;
      });

      document.getElementById('readme-loop-count').textContent = \`\${filtered.length} items\`;

      if (filtered.length === 0) {
        document.getElementById('readme-loop-table').style.display = 'none';
        document.getElementById('readme-loop-empty').style.display = 'block';
      } else {
        document.getElementById('readme-loop-table').style.display = 'table';
        document.getElementById('readme-loop-empty').style.display = 'none';

        filtered.forEach(repoWithOwner => {
          const state = data.readmeState.repositories[repoWithOwner];
          const tr = document.createElement('tr');

          const branchName = state.branch || \`readme-review-\${repoWithOwner.split('/')[1]}\`;
          
          let prCell = '';
          if (state.pr_number) {
            const prUrl = \`https://github.com/\${repoWithOwner}/pull/\${state.pr_number}\`;
            prCell = \`<a href="\${prUrl}" target="_blank" class="link-primary">PR #\${state.pr_number} ↗</a>\`;
          } else {
            prCell = \`<span style="color: var(--text-muted)">None (Not Started / Merged)</span>\`;
          }

          let progressBadgeClass = 'badge-neutral';
          let statusText = 'Idle';
          
          if (state.pr_number) {
            if (state.cycles >= 3 && state.applied_cycles >= 3) {
              progressBadgeClass = 'badge-success';
              statusText = \`Finished & LGTM (\${state.cycles}/3 Turns)\`;
            } else if (state.cycles > state.applied_cycles) {
              progressBadgeClass = 'badge-warning';
              statusText = \`Refining README... (\${state.cycles}/3)\`;
            } else {
              progressBadgeClass = 'badge-info';
              statusText = \`Awaiting Reviewer (\${state.cycles}/3)\`;
            }
          } else {
            statusText = 'Idle';
          }
          ...
          tr.innerHTML = \`
            <td><strong>\${repoWithOwner}</strong></td>
            <td><code>\${branchName}</code></td>
            <td>\${prCell}</td>
            <td>
              <span class="badge \${progressBadgeClass}">\${statusText}</span>
            </td>
          \`;
          tbody.appendChild(tr);
        });
